...
function FfmpegProcessor(source) {
  ...
  FfmpegProcessor.prototype.renice = function(level) {
  	if (!level) {
            ...
            async.until(
              ...
              function(taskcallback) {
                ...
                if (self.options._nice.level) {
                	...
                	exec('nice --adjustment="' + self.options._nice.level + '" ffmpeg ' + tnArgs.join(' '), taskcallback);7
                } else {                	
  ...
  FfmpegProcessor.prototype._spawnProcess = function(args) {
  	var retProc = spawn('ffmpeg', args);
