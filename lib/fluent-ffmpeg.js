...
function FfmpegCommand(args) {
  ...
  FfmpegCommand.prototype.usingPreset = function(preset, opt_options) {
    if (typeof preset === 'function') {
      preset(this, opt_options);
    } else {
  ...
  FfmpegCommand.prototype._prepare = function(callback) {
    ...
    if (calcDimensions || calcPadding || this.requiresMetaData()) {
      this.getMetadata(this.options.inputfile, function(meta) {
        self.metaData = meta;
        var dimErr, padErr;
        ...
        // calculate padding
        if (calcPadding) {
          self._calculatePadding(meta);
        }

        if (dimErr) {
          callback(dimErr);
        } else {
          self._checkCapabilities(callback);
        }
      });
    } else {
      callback(undefined);
