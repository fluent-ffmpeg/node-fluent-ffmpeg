...
exports = module.exports = function Metadata(FfmpegCommand) {
  ...
  // Implement ffmpeg stream selection algorithm (according to ffmpeg documentation)
  // The returned object has a truthy notFound property if no stream was found
  function selectStream(streams, type) {
    switch(type) {
      case 'video':
        // Return stream with greatest resolution
        return streams.reduce(function(biggest, stream) {
          if (stream.codec_type === 'video' && biggest.width * biggest.height < stream.width * stream.height) {
            return stream;
          } else {
            return biggest;
          }
        }, { notFound: true, width: 0, height: 0 });

      case 'audio':
        // Return stream with most channels
        return streams.reduce(function(most, stream) {
          if (stream.codec_type === 'audio' && most.channels < stream.channels) {
            return stream;
          } else {
            return most;
          }
        }, { notFound: true, channels: 0 });

      case 'subtitles':
        // Return first stream
        return streams.filter(function(stream) {
          return stream.codec_type === 'subtitles';
        })[0] || { notFound: true };
  ...
  // Get metadata required by size calculations from ffprobe
  FfmpegCommand.prototype.getRequiredMetadata = function(callback) {
    if (this._requiredMetadata) {
      callback(null, this._requiredMetadata);
    } else {
      var self = this;

      this.ffprobe(function(err, data) {
        if (err) {
          return callback(err);
        }
        ...
        // Get duration
        if (typeof data.format.duration === 'string' && data.format.duration.indexOf(':') !== -1) {
          ret.durationsec = self.ffmpegTimemarkToSeconds(data.format.duration);
        } else {
          ret.durationsec = Number(data.format.duration);
        }

        // Select video stream
        var vstream = selectStream(data.streams, 'video');
        if (!vstream.notFound) {
          ret.video.resolution.w = vstream.width;
          ret.video.resolution.h = vstream.height;

          var aspect = vstream.display_aspect_ratio;
          var pixel = vstream.pixel_aspect_ratio || vstream.sample_aspect_ratio;
          var n;

          // save aspect ratio for auto-padding
          if (aspect) {
            ret.video.aspectString = aspect;
            n = aspect.split(':');
  ...
  FfmpegCommand.prototype.meta = function() {
    if (this.metaData){
  ...
  FfmpegCommand.prototype.requiresMetaData = function() {
