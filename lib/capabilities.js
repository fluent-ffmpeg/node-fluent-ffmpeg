  ...
  proto.getAvailableFormats = function(callback) {
    ...
    this._spawnFfmpeg(['-formats'], { captureStdout: true }, function (err, stdout) {
      ...
      lines.forEach(function(line) {
        ...
        if (match) {
          // Split the format name by comma
          var parsedFormat = match[3].split(",");
          for (var i = 0; i !== parsedFormat.length; i++) {
            // If the format exists only change false values and keep
            // the description
            if (data[parsedFormat[i]]) {
              if (!data[parsedFormat[i]]['canDemux']) {
                data[parsedFormat[i]]['canDemux'] = match[1] ==='D';
              }
              if (!data[parsedFormat[i]]['canMux']) {
                data[parsedFormat[i]]['canMux'] = match[2] ==='E';
              }
            }
            // If the format doesn't exist add it normally
            else {
              data[parsedFormat[i]] = {
                description: match[4],
                canDemux: match[1] === 'D',
