...
module.exports = function(proto) {
   ...
   *
   * @callback FfmpegCommand~deviceCallback
   * @param {Error|null} err error object or null if no error happened
   * @param {Object} devices device object with device names as keys and the following
   *   properties for each device:
   * @param {String} devices.description device description
   * @param {Boolean} devices.canDemux whether the device is able to demux streams from an input device
   * @param {Boolean} devices.canMux whether the device is able to mux streams into an output device
   */
   ...
   *
   * @param {FfmpegCommand~deviceCallback} callback callback function
   */
  ...
  proto.getAvailableDevices = function(callback) {
    ...
    // Run ffmpeg -devices
    this._spawnFfmpeg(['-devices'], { captureStdout: true }, function (err, stdout) {
      if (err) {
        return callback(err);
      }
      ...
      lines.forEach(function(line) {
        var match = line.match(deviceRegexp);
        if (match) {
