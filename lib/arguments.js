  ...
  FfmpegCommand.prototype.getSizeFilters = function() {
    ...
    if (percentRatio) {
      ...
      if (this.options.video.aspect) {
        ...
        // -2 tells the scale filter to automatically compute the other dimension
        // while keeping it divisible by 2
        if (fixedWidth) {
          return ['scale=' + (Math.round(Number(fixedWidth[1]) / 2) * 2) + ':-2'];
        } else {
          return ['scale=-2:' + (Math.round(Number(fixedHeight[1]) / 2) * 2)];
        }
      }
    } else {
