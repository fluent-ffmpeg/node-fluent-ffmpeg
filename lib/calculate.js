...
exports = module.exports = function calculate(FfmpegCommand) {
  FfmpegCommand.prototype._aspectIsEqual = function(ar1, ar2) {
  ...
  FfmpegCommand.prototype._calculatePadding = function(data) {
  ...
  FfmpegCommand.prototype._calculateDimensions = function(data) {
    ...
    function makeError(message) {
      ...
      err.ffmpegErr = data.ffmpegErr;
      err.ffmpegOut = data.ffmpegOut;
      err.spawnErr = data.spawnErr;

      return err;
    }

    if (!resolution) {
      return makeError('could not determine video resolution, check your ffmpeg setup');
    }
    ...
    if (fixedWidth && fixedWidth.length > 0) {
      ...
      if (!resolution.w) {
        return makeError('could not determine width of source video, aborting execution');
      }
      ...
      if (this.options.video.aspect !== undefined) {
        ...
        if (ardata) {
          ...
          return makeError('could not parse aspect ratio set using withAspect(), aborting execution');
        }
      } else {
        w = parseInt(fixedWidth[1], 10);
      ...
      if (!resolution.h) {
        return makeError('could not determine height of source video, aborting execution');
      }
      ...
      if (!resolution.w || !resolution.h) {
        return makeError('could not determine resolution of source video, aborting execution');
      }
      ...
      return makeError('could not determine type of size string, aborting execution');
    }
  ...
  FfmpegCommand.prototype.calculateDimensions = FfmpegCommand.prototype._calculateDimensions;
};
