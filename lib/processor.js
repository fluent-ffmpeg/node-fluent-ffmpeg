...
module.exports = function(proto) {
  ...
  proto.run = function() {
    ...
    var ended = false;
    function emitEnd(err, stdout, stderr, pid = null) {
      if (!ended) {
        ended = true;

        if (err) {
          self.emit('error', err, stdout, stderr, pid);
        } else {
          self.emit('end', stdout, stderr, pid);
        }
      }
    }
            ...
            if (flvmeta.length) {
              self._getFlvtoolPath(function(err, flvtool) {
                ...
                async.each(
                  ...
                  function(err) {
                    if (err) {
                      emitEnd(err, null. null, pid);
                    } else {
                      emitEnd(null, stdoutRing.get(), stderrRing.get(), pid);
                    }
                  }
                );
              });
