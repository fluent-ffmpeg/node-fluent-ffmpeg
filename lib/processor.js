    ...
    this._prepare(function(err) {
      ...
      ffmpegProc.stderr.on('data', function (data) {
        ...
        if (options.onProgress) {
          self._getProgressFromStdErr(stderr, this.metaData.durationsec);
        }
      });
    });
  };
        ...
        if (Array.isArray(timemarks)) {
          for (var i = 0; i < timemarks.length; i++) {
            ...
            if( timemarks[i].indexOf('%') > 0 ) {
              timemarks[i] = (parseInt(timemarks[i], 10) / 100) * this.metaData.durationsec;
            }
            if (parseInt(timemarks[i], 10) > this.metaData.durationsec) {
          ...
          if (timemarks.length === 0) {
            timemarks[0] = (this.metaData.durationsec * 0.9);
          }
        }
        // get positions for screenshots (using duration of file minus 10% to remove fade-in/fade-out)
        var secondOffset = (this.metaData.durationsec * 0.9) / screenshotcount;
        var donecount = 0;
        var series = [];
