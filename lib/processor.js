...
module.exports = function(proto) {
   ...
   * @param {Number} [progress.inputBytesRead] number of bytes read from the input stream (only available for input streams)
   * @param {Number} [progress.inputBytesLength] total number of bytes in the input stream (only available for input streams, if provided by calling '.inputStreamLength')
   * @param {Number} [progress.inputReadPercent] input stream read progress (only available for input streams, only present if `progress.inputBytesLength` is also present)
   */
        ...
        function processCB(ffmpegProc, stdoutRing, stderrRing) {
          ...
          if (stderrRing) {
            ...
            if (self.listeners('progress').length) {
              ...
              if (inputStream) {
                // If there's any progress listeners also listen for data events from the 
                // input stream
                inputStream._bytesRead = 0;
                inputStream.source.on('data', function(data) {
                  inputStream._bytesRead += data.length;
                });
              }
				  
              stderrRing.callback(function(line) {
                var progress = utils.extractProgress(line);
                
                if (progress) {
                  // calculate percent progress using duration
                  var ffprobeData = self._ffprobeData;
                  if (ffprobeData && ffprobeData.format && ffprobeData.format.duration) {
                    var duration = Number(ffprobeData.format.duration);
                    if (!isNaN(duration))
                      progress.percent = (utils.timemarkToSeconds(progress.timemark) / duration) * 100;
                  }
                  
                  if (inputStream) {
                    // Add input stream progress
                    progress.inputBytesRead = inputStream._bytesRead;
                    if (inputStream._streamLength) {
                      // Only include if we've provided a length
                      progress.inputBytesLength = inputStream._streamLength;
                      progress.inputReadPercent = (progress.inputBytesRead / progress.inputBytesLength) * 100;
                    }
                  }
                }
