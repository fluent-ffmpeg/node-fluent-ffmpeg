...
exports = module.exports = function Processor(command) {
    ...
    this._prepare(function(err, meta) {        
      if (err) {
        ...
        ffmpegProc.stderr.on('data', function (data) {
          stderr += data; 
          if (self.options.informAudioCb) self._checkAudioInput(stderr);
          if (self.options.informVideoCb) self._checkVideoInput(stderr);
        });
      }
    });
  };
  ...
  this._checkAudioInput = function(stderr) {
    var audio = /Audio: ([a-zA-Z0-9]+),/.exec(stderr);
    if (audio && audio.length > 1) { //first item is matched text
      this.options.informAudioCb(audio[1]);
      this.options.informAudioCb = null;
    }
  };
  
  this._checkVideoInput = function(stderr) {
    var video = /Video: ([a-zA-Z0-9]+),/.exec(stderr);
    if (video && video.length > 1) { //first item is matched text
      this.options.informVideoCb(video[1]);
      this.options.informVideoCb = null;
    }  
  };
  ...
  this._spawnProcess = function(args, options) {
    ...
    if (retProc.stdout) retProc.stdout.setEncoding('utf8');
