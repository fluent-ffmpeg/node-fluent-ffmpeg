...
exports = module.exports = function processor(FfmpegCommand) {
  ...
  FfmpegCommand.prototype.takeScreenshots = function(config, folderOrStream, callback) {
      ...
      self._prepare(function(err, args) {
        ...
        async.until(
          ...
          function(taskcallback) {
            ...
            var target = path.join(folderOrStream, fname);
    ...
    // check target folder
    check(folderOrStream, function(exists) {
      if (!exists) {
        fs.mkdir(folderOrStream, '0755', function(err) {
          if (err !== null) {
            self.emit('error', err);
          } else {
            _screenShotInternal();
