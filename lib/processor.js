...
module.exports = function(proto) {
  ...
  proto._getArguments = function() {
    ...
    if (complexFilters.length) {
      // Merge 'simple' filters into complex filters
      // (some ffmpeg versions don't support both at the same time)
      var filters = audioFilters.concat(videoFilters);
      var map = complexFilters.indexOf('-map');

      if (map !== -1) {
        // Skip first item ('-filter_complex') and '-map' options
        filters = filters.concat(complexFilters.slice(1, map));
      } else {
        // Just skip first item ('-filter_complex')
        filters = filters.concat(complexFilters.slice(1));
      }

      complexFilters = ['-filter_complex', filters.join(';')]
        // Re-append map options if present
        .concat(map === -1 ? [] : complexFilters.slice(map));
