...
module.exports = function(proto) {
  ...
  proto._spawnFfmpeg = function(args, options, processCB, endCB) {
    ...
    this._getFfmpegPath(function(err, command) {
      ...
      // Apply niceness
      if (options.niceness && !utils.isWindows) {
   ...
   * @param {Number} [niceness] niceness value between -20 and 20, defaults to 0
   * @return FfmpegCommand
   */
  proto.renice = function(niceness) {
    if (!utils.isWindows) {
      ...
      if (this.ffmpegProc) {
        ...
        var renice = spawn('renice', ['-n', niceness > 0 ? '+' + niceness : niceness, '-p', pid]);

        renice.on('error', function(err) {
          logger.warn('could not renice process ' + process.pid + ': ' + err.message);
        });
