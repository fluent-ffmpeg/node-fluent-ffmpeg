...
module.exports = function(proto) {
          ...
          if (err) {
            ...
            if (flvmeta.length) {
              self._getFlvtoolPath(function(err, flvtool) {
                // No possible error here, _getFlvtoolPath was already called by _prepare
                async.each(
                  flvmeta,
                  function(output, cb) {
  ...
  proto.mergeToFile = function(targetfile) {
    ...
    function makeIntermediateFile(_mergeSource,_callback) {
        ...
        var args = self._currentOutput.options.get().concat(['-i', _mergeSource, '-qscale:v', 1, fname]);
  ...
  proto.takeScreenshots = function(config, folder) {
    ...
    function _screenShotInternal() {
      self._prepare(function(err, args) {
        ...
        _computeSize(self._currentOutput.sizeData.size);
    ...
    if (!this._currentOutput.sizeData || !this._currentOutput.sizeData.size) {
