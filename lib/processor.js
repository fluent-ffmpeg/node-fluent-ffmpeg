...
exports = module.exports = function Processor(command) {
  ...
  this._spawnProcess = function(args, options) {
    ...
    if (this.options.onProgress && typeof this.options.onProgress == "function") {
        retProc.stderr.on('data', function (err) {
            var error = err.toString();
            if (error.match(/^frame/)) {
                // frame=  653 fps= 35 q=0.0 size=   43494kB time=27.21 bitrate=13095.3kbits/;
                var matches = error.match(/frame=\s*([^\s]+)\s+fps=\s*([^\s]+)\s+q=\s*([^\s]+)\s+size=\s*([^\s]+)\s+time=\s*([^\s]+)\s+bitrate=\s*([^\s]+)/);

                if (matches) {
                    this.options.onProgress({
                        frame: parseInt(matches[1], 10),
                        fps:   parseInt(matches[2], 10),
                        q:     matches[3],
                        size:  matches[4],
                        time:  matches[5]
                    });
  ...
  this.buildFfmpegArgs = function(overrideOutputCheck) {
    ...
    if (this.options.inputfile && !this.options.inputstream) {
      try
      {
        ...
        } else if (/%/.exec(this.options.inputfile)) { // multi-file format - http://ffmpeg.org/ffmpeg-doc.html#SEC18
    ...
    // check for extra input files - should come before other arguments
    if (this.options.otherInputs && Array.isArray(this.options.otherInputs) && this.options.otherInputs.length > 0) {
