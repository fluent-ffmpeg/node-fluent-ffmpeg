...
/**
   * Adds multiple inputs from an array
   *
   * @method FfmpegCommand#inputPathArray
   * @category Input
   *
   * @param {String[]} source Array of file paths
   * @return FfmpegCommand
   */
  proto.inputPathArray = function(source) {
    var sourcePathArrConvert = Array.prototype.slice.call(source);

    sourcePathArrConvert.forEach(path => {
      this._inputs.push(this._currentInput = {
        source: path,
        isFile: true,
        isStream: false,
