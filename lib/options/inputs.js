...
module.exports = function(proto) {
  ...
  /**
   * Specify input stream size for the last specified input stream
   *
   * @method FfmpegCommand#inputStreamLength
   * @category Input
   * @aliases withInputStreamLength
   *
   * @param {Number} length input stream length
   * @return FfmpegCommand
   */
  proto.withInputStreamLength =
  proto.inputStreamLength = function(length) {
    if (!this._currentInput) {
      throw new Error('No input specified');
    }
    
    if (!this._currentInput.isStream) {
      throw new Error('Last input was not a stream');
    }
    
    this._currentInput._streamLength = length
    return this;
  };
};
